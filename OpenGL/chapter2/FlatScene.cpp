...
void FlatScene::compileAndLinkShader()
{
    if (!prog.compileShaderFromFile("chapter2\\Shader\\FlatShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter2\\Shader\\FlatShader.frag", GLSLShader::FRAGMENT))
    {
...
FlatScene::FlatScene() : angle(0.0f)
{
}

void FlatScene::initScene()
{
