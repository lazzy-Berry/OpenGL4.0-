...
{
    if (!prog.compileShaderFromFile("chapter3\\SpotLight\\Shader\\SpotLightShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter3\\SpotLight\\Shader\\SpotLightShader.frag", GLSLShader::FRAGMENT))
    {
...
void SceneSpotLight::resize(int w, int h)
{
    glViewport(0, 0, w, h);
    width = w;
    height = h;
