...
void SceneDefferd::compileAndLinkShader()
{
    if (!prog.compileShaderFromFile("chapter5\\Defferd\\Shader\\DefferdShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter5\\Defferd\\Shader\\DefferdShader.frag", GLSLShader::FRAGMENT))
    {
...
void SceneDefferd::render()
{
    pass1();
    pass2();
}

void SceneDefferd::update(float t)
{
    angle += 0.01f;
    if (angle > TWOPI) angle -= TWOPI;
}

void SceneDefferd::resize(int w, int h)
{
    glViewport(0, 0, w, h);
    width = w;
    height = h;
