...
VBOMesh7::VBOMesh7(const char* fileName, bool center, bool loadTc, bool genTangents) :
    reCenterMesh(center), loadTex(loadTc), genTang(genTangents)
{
    loadOBJ(fileName);
}

void VBOMesh7::render() const {
    glBindVertexArray(vaoHandle);
    glDrawElements(GL_TRIANGLES, 3 * faces, GL_UNSIGNED_INT, ((GLubyte*)NULL + (0)));
}

void VBOMesh7::loadOBJ(const char* fileName) {
...
void VBOMesh7::center(vector<vec3>& points) {
...
void VBOMesh7::generateAveragedNormals(
...
void VBOMesh7::generateTangents(
    const vector<vec3>& points,
...
void VBOMesh7::storeVBO(const vector<vec3>& points,
    const vector<vec3>& normals,
    const vector<vec2>& texCoords,
...
void VBOMesh7::trimString(string& str) {
