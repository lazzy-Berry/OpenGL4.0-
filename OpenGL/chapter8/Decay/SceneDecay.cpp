...
{
    if (!prog.compileShaderFromFile("chapter8\\Decay\\Shader\\DecayShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter8\\Decay\\Shader\\DecayShader.frag", GLSLShader::FRAGMENT))
    {
...
{ 
    ...
    GLuint noiseTex = SeamlessNoiseTex::generate2DTex(true);
