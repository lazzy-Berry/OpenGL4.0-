...
{
    if (!prog.compileShaderFromFile("chapter9\\ParticlesFeedback\\Shader\\ParticlesFeedbackShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter9\\ParticlesFeedback\\Shader\\ParticlesFeedbackShader.frag", GLSLShader::FRAGMENT))
    {
...
void SceneParticlesFeedback::initBuffers()
{
    ...
    for (int i = 0; i < nParticles; i++) data[i] = 0.0f;
    glBindBuffer(GL_ARRAY_BUFFER, posBuf[0]);
