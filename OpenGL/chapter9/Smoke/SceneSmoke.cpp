...
{
    if (!prog.compileShaderFromFile("chapter9\\Smoke\\Shader\\SmokeShader.vert", GLSLShader::VERTEX))
    {
        printf("Vertex shader failed to compile!\n%s",
            prog.log().c_str());
        exit(1);
    }
    if (!prog.compileShaderFromFile("chapter9\\Smoke\\Shader\\SmokeShader.frag", GLSLShader::FRAGMENT))
    {
...
{ 
    ...
    const char* texName = "chapter9\\Texture\\smoke.png";
